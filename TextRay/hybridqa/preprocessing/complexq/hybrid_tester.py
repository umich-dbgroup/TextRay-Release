...
class Tester_Interface(object):
    ...
    def get_average_f1(self, src, n_best=25):
        ...
        for qid, group in df.groupby("qid"):
            ...
            # print(group_df_sub)
            best_row = group_df_sub.loc[group_df_sub['precision'].idxmax()]
            question = self.questions_dict[qid]
            ground_answers = question["Answers"]
            all_ground += len(ground_answers)
            preds = best_row['pred_entities']
            all_predicted += len(preds)
            for entity in preds:
                if entity in ground_answers:
                    precision += 1
            for entity in ground_answers:
                if entity in preds:
                    recall += 1
        precision = float(precision) / float(all_predicted)
        recall = float(recall) / float(all_ground)
        results = {}
        if precision > 0 and recall > 0:
            f1 = (2 * precision * recall) / (precision + recall)
            results["micro"] = (precision, recall, f1)
        else:
            results["micro"] = (0.0, 0.0, 0.0)
        all_f1s = []
        all_precision = []
        all_recall = []
        prec_1 = 0
        ...
        macro_avg_f1 = float(sum(all_f1s)) / len(self.questions_dict)
        prec_at_1 = float(prec_1) / len(self.questions_dict)
...
if __name__ == '__main__':
    ...
    # RUNNING_DIR = "/media/nbhutani/Data/textray_workspace/emnlp_data/a5_wo_constraints"
    # RUNNING_DIR = "/media/nbhutani/Data/textray_workspace/emnlp_data/a5_wo_prior"
    # RUNNING_DIR = "/media/nbhutani/Data/textray_workspace/emnlp_data/a5_dev"
    # RUNNING_DIR = "/media/nbhutani/Data/textray_workspace/emnlp_data/a5_wo_attention"
    # RUNNING_DIR = "/media/nbhutani/Data/textray_workspace/emnlp_data/a5_dev"
